...
await delay(5 * 5000)
await conn.sendMessage(res1, { text: text + '\n\n_atte. 𝙎𝙝𝙖𝙧𝙠𝙨𝘽𝙤𝙩-𝙈𝘿_', mentions: (await conn.groupMetadata(`${res1}`)).participants.map(v => v.id) }, { quoted: fkontak })
await conn.sendMessage(res2, { text: text + '\n\n_atte. 𝙎𝙝𝙖𝙧𝙠𝙨𝘽𝙤𝙩-𝙈𝘿_', mentions: (await conn.groupMetadata(`${res2}`)).participants.map(v => v.id) }, { quoted: fkontak })
await conn.sendMessage(res3, { text: text + '\n\n_atte. 𝙎𝙝𝙖𝙧𝙠𝙨𝘽𝙤𝙩-𝙈𝘿_', mentions: (await conn.groupMetadata(`${res3}`)).participants.map(v => v.id) }, { quoted: fkontak })
await conn.sendMessage(res4, { text: text + '\n\n_atte. 𝙎𝙝𝙖𝙧𝙠𝙨𝘽𝙤𝙩-𝙈𝘿_', mentions: (await conn.groupMetadata(`${res4}`)).participants.map(v => v.id) }, { quoted: fkontak })
await conn.sendMessage(res5, { text: text + '\n\n_atte. 𝙎𝙝𝙖𝙧𝙠𝙨𝘽𝙤𝙩-𝙈𝘿_', mentions: (await conn.groupMetadata(`${res5}`)).participants.map(v => v.id) }, { quoted: fkontak })
