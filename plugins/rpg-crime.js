//CÓDIGO CREADO POR elrebelde21 : https://github.com/BryantPPT
let crime = 500
let diamante = 10
...
if (global.db.data.users[m.sender].money < 0) return m.reply(`《💰》${pickRandom(global.robar)} ${money} 🪙 SharksCoins`).catch(global.db.data.users[m.sender].money += money) 
...
if (media === 'text3') return m.reply(`《💰》${pickRandom(global.robar)}\n\n${diamond} 💎 𝐃𝐈𝐀𝐌𝐀𝐍𝐓𝐄\n${money} 🪙 𝐒𝐇𝐀𝐑𝐊𝐒𝐂𝐎𝐈𝐍𝐒`).catch(global.db.data.users[m.sender].limit += diamond).catch(global.db.data.users[m.sender].money += money)
